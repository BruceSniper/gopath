...
func main() {
	...
	*/
	...
	fun3(arr1)
	fmt.Println("fun3()函数调用后，a:", arr1)

	fun4(&arr1)
	fmt.Println("fun4()函数调用后，a:", arr1)
...
func fun1(num int) { //值传递：num = a = 10
	...
	num = 100
	fmt.Println("fun1函数中修改num，num的值：", num)
}
...
func fun3(arr2 [4]int) { //值传递
	...
	arr2[0] = 100
	fmt.Println("fun3()函数中修改数据：", arr2)
}

func fun4(p2 *[4]int) { //引用传递，传递的是地址
