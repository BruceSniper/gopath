package main

import (
	"fmt"
	"io/ioutil"
	"os"
)

func main() {
	/*
		ioutil包：
			ReadFile()
			WriteFile()
			ReadDir()
			...
	*/
	//1.读取文件中的所有的数据
	//fileName := "E:/gopath/src/l_file/aa.txt"
	//data, err := ioutil.ReadFile(fileName)
	//fmt.Println(err)
	//fmt.Println(data)
	//fmt.Println(string(data))

	//2.写出数据
	//fileName := "E:/gopath/src/l_file/bbb.txt"
	////s1 := "hello world 面朝大海，春暖花开"
	//s1 := "床前明月光"
	//err := ioutil.WriteFile(fileName, []byte(s1),os.ModePerm)
	//fmt.Println(err)

	//3.ReadAll()
	//s2 := "王二狗和李小花"
	//r1 := strings.NewReader(s2)
	//data,err := ioutil.ReadAll(r1)
	//fmt.Println(err)
	//fmt.Println(data)
	//fmt.Println(string(data))

	//4.ReadDir()，读取一个目录下的字内容：子文件和子目录，但是只能读取一层
	//dirName := "E:/Agriculture_KnowledgeGraph_master/demo"
	//fileInfos, err := ioutil.ReadDir(dirName)
	//if err != nil {
	//	fmt.Println(err)
	//	return
	//}
	//fmt.Println(len(fileInfos))
	//for i := 0; i < len(fileInfos); i++ {
	//	//fmt.Printf("%T\n",fileInfos[i])
	//	fmt.Printf("第%d个：名称：%s，是否是目录：%t\n",i+1,fileInfos[i].Name(),fileInfos[i].IsDir())
	//}

	//5.临时目录和临时文件
	//dir, err := ioutil.TempDir("E:/gopath/src/l_file/","Test")
	//if err != nil{
	//	fmt.Println(err)
	//	return
	//}
	//defer os.Remove(dir)
	//fmt.Println(dir)

	file, err := ioutil.TempFile("E:/gopath/src/l_file/", "Test")
	if err != nil {
		fmt.Println(err)
		return
	}
	defer os.Remove(file.Name())
	fmt.Println(file.Name())

}
