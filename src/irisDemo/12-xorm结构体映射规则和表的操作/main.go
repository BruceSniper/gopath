package main

import (
	"fmt"
	_ "github.com/go-sql-driver/mysql" //不能忘记导入
	"xorm.io/xorm"
	"xorm.io/xorm/names"
)

func main() {
	//1.创建数据库引擎对象
	engine, err := xorm.NewEngine("mysql", "root:1892@/testcms?charset=utf8")
	if err != nil {
		panic(err.Error())
	}

	//设置名称映射规则
	engine.SetMapper(names.SnakeMapper{}) //将对象的驼峰式命名修改为小写，中间用_分隔
	engine.Sync2(new(UserTable))

	//engine.SetMapper(names.SameMapper{}) //将结构体对象不修改命名，写入数据表
	engine.Sync2(new(StudentTable))

	//names.GonicMapper{}对一些特定的字段处理比较友好，不会在字段中加_，比如id
	//engine.SetMapper(names.GonicMapper{})
	engine.Sync2(new(PersonTable))

	PersonEmpty, err := engine.IsTableEmpty(new(PersonTable))
	if err != nil {
		panic(err.Error())
	}
	if PersonEmpty {
		fmt.Println(" 人员表是空的 ")
	} else {
		fmt.Println(" 人员表不为空 ")
	}

	studentExit, err := engine.IsTableExist(new(StudentTable))
	if studentExit {
		fmt.Println(" 学生表存在 ")
	} else {
		fmt.Println(" 学生表不存在 ")
	}
}

type UserTable struct {
	UserId   int64  `xorm:"pk autoincr"` //pk = primary key，自增
	UserName string `xorm:"varchar(32)"` //用户名，可变长度，不高于32位
	UserAge  int64  `xorm:"default 1"`   //用户年龄，默认值1
	UserSex  int64  `xorm:"default 0"`   //用户性别，默认值0
}

/**
 * 学生表
 */
type StudentTable struct {
	Id          int64  `xorm:"pk autoincr"` //主键 自增
	StudentName string `xorm:"varchar(24)"` //
	StudentAge  int    `xorm:"int default 0"`
	StudentSex  int    `xorm:"index"` //sex为索引
}

/**
 * 人员结构表
 */
type PersonTable struct {
	Id         int64     `xorm:"pk autoincr"`   //主键自增
	PersonName string    `xorm:"varchar(24)"`   //可变字符
	PersonAge  int       `xorm:"int default 0"` //默认值
	PersonSex  int       `xorm:"notnull"`       //不能为空
	City       CityTable `xorm:"-"`             //不映射该字段
}

type CityTable struct {
	CityName      string
	CityLongitude float32
	CityLatitude  float32
}
